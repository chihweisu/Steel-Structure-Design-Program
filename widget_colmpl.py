...
class colmplwidget(QtWidgets.QWidget):
    def __init__(self, parent=None):
        QtWidgets.QWidget.__init__(self, parent)
        self.canvas = MplCanvas()
        self.canvas.axes = self.canvas.figure.add_subplot(111,facecolor='white')
        self.canvas.axes.set_xlabel('MuX(tf-m)', fontsize=8, color='white')
        self.canvas.axes.set_ylabel('MuY(tf)', fontsize=8, color='white')
